...
*/
  ...
  */
  ...
  AuthenticationContext.prototype._isPopup = function () {
      return this._openedWindows.length !== 0
          && this._openedWindows[this._openedWindows.length - 1].opener != null
          && this._openedWindows[this._openedWindows.length - 1].opener._adalInstance != null
  };

  AuthenticationContext.prototype._isFrame = function () {
      return window !== window.parent && !this._isPopup();
  }

  AuthenticationContext.prototype._expectCallbackMessage = function () {
      var ctx = this;
      window.addEventListener('message', function cb(e) {
          if (e.data.hash != null && ctx.isCallback(e.data.hash)) {
              ctx.handleWindowCallback(e.data.hash);

              // stop expecting messages
  ...
  AuthenticationContext.prototype.handleWindowCallback = function (hash) {
      ...
      if (this.isCallback(hash)) {
          if (this._isPopup()) {
              ...
              mainWindow.postMessage({ hash }, window.origin);
          } else if (this._isFrame()) {
              var mainWindow = window.parent;
              mainWindow.postMessage({ hash }, window.origin);
          } else {
              var self = window._adalInstance;
